...
void assign_particles(region &r, particle_t *particles, const int n_particles){
    ...
    for(int i = 0; i < n_particles; ++i){
        ...
        }else if((part.x >= r.helper_x_min) && (part.x < r.helper_x_max) && (part.y >= r.helper_y_min) && (part.y < r.helper_y_max)){
...
void copy_region_to_host(region &r){
    ...
    cudaMemcpy(r.h_helper_particles, r.d_helper_particles, r.n_helper_particles * sizeof(particle_t), cudaMemcpyDeviceToHost);
    cudaDeviceSynchronize();
}
